WITH CHARGE_CONTROL AS (
  	SELECT 
			...
			CASE WHEN "public"."property_charge_controls"."item_id" = 'TRS' then 1 else 0 end as "TRASH", 
			CASE WHEN "public"."property_charge_controls"."item_id" = 'PET' then 1 else 0 end as "PET",
			CASE WHEN NOT "public"."property_charge_controls"."base_rent"
								AND "public"."property_charge_controls"."item_id" != 'TRS'
								AND "public"."property_charge_controls"."item_id" != 'PET'
...
CHARGES_TOT AS (
  SELECT 
		...
		CASE WHEN CHARGE_CONTROL."TRASH" = 1 THEN "public"."lease_recurring_charge_amounts"."amount" ELSE 0 END AS "TRASH_AMOUNT",
		CASE WHEN CHARGE_CONTROL."PET" = 1 THEN "public"."lease_recurring_charge_amounts"."amount" ELSE 0 END AS "PET_AMOUNT",
  		CASE WHEN CHARGE_CONTROL."OTHER" = 0 THEN "public"."lease_recurring_charge_amounts"."amount" ELSE 0 END AS "OTHER_CHARGE",
...
LEASES_CHARGES AS (
	...
	MAX(LEASES."lease_end") "lease_end", --because of difference in seconds, can't be grouped
  	...
  	SUM(CASE WHEN CHARGES."FREQUENCY" = 'Annually' THEN CHARGES."PET_AMOUNT" / 12 ELSE CHARGES."PET_AMOUNT" END) AS "PET_AMOUNT",
	SUM(CASE WHEN CHARGES."FREQUENCY" = 'Annually' THEN CHARGES."TRASH_AMOUNT" / 12 ELSE CHARGES."TRASH_AMOUNT" END) AS "TRASH_AMOUNT",
	...
	SUM(CASE WHEN CHARGES."FREQUENCY" = 'Annually' THEN CHARGES."RENT_CHARGE" ELSE CHARGES."RENT_CHARGE" * 12 END) AS "ANNUAL_RENT_AMOUNT",
  	SUM(CASE WHEN CHARGES."FREQUENCY" = 'Annually' THEN CHARGES."PET_AMOUNT" ELSE CHARGES."PET_AMOUNT" * 12 END) AS "ANNUAL_PET_AMOUNT",
	SUM(CASE WHEN CHARGES."FREQUENCY" = 'Annually' THEN CHARGES."TRASH_AMOUNT" ELSE CHARGES."TRASH_AMOUNT" * 12 END) AS "ANNUAL_TRASH_AMOUNT"
...
SQ_FT_ITEMS AS (
  ...
  FROM (
	  SELECT 
		...
		"value",
		"as_of_date",
		ROW_NUMBER() OVER (PARTITION BY "unit_id" ORDER BY "as_of_date" DESC) AS "rn"
	  FROM "public"."unit_square_footage_items"  --if implemented on a std report, filter by company_id
...
UNITS AS (
  SELECT 
    ...
    MAX(COALESCE(uq."value", '0')) AS "UNIT_SQ_FT"
  
  FROM "public"."units"
  INNER JOIN "public"."properties"
    ON "public"."units"."property_id" = "public"."properties"."id"
  LEFT JOIN SQ_FT_ITEMS AS uq
...
FINAL."RENT_AMOUNT" "PET_AMOUNT",
FINAL."RENT_AMOUNT" "TRASH_AMOUNT",
--FINAL."OTHER_AMOUNT" "OTHER_AMOUNT",
FINAL."ANNUAL_RENT_AMOUNT" "ANNUAL_RENT_AMOUNT",
FINAL."ANNUAL_RENT_AMOUNT" "ANNUAL_PET_AMOUNT",
FINAL."ANNUAL_RENT_AMOUNT" "ANNUAL_TRASH_AMOUNT",
...
CASE WHEN UNITS."UNIT_SQ_FT" = 0 THEN 0 ELSE FINAL."PET_AMOUNT" *12 /UNITS."UNIT_SQ_FT" END AS "Annual Pet/Sq Ft",
CASE WHEN UNITS."UNIT_SQ_FT" = 0 THEN 0 ELSE FINAL."TRASH_AMOUNT" *12 /UNITS."UNIT_SQ_FT" END AS "Annual Trash/Sq Ft"
...
order by "PROP_NAME", UNITS."UNIT_NAME" ASC