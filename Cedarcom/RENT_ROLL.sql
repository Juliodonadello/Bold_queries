...
MAX_LEASES AS (
   ...
   "UNIT_ID"
 	FROM LEASES_TOT
	GROUP BY "UNIT_ID" --debería ser agrupado por LEASE_ID pero la base está muy mal, agrupo por unit_id para tener un dataset util y limpio
 ),
LEASES AS ( 
 SELECT  LEASES_TOT.*
 FROM  LEASES_TOT
 INNER JOIN MAX_LEASES
 	ON  --LEASES_TOT."LEASE_ID" =  MAX_LEASES."LEASE_ID" 	AND  
	LEASES_TOT."UNIT_ID" =  MAX_LEASES."UNIT_ID"
...
SELECT 
	...
	MAX(LEASES."lease_end") "lease_end", --because of difference in seconds, can't be grouped
  	...
  	COALESCE(MAX(CASE WHEN LEASES."REFUNDABLE" = 'true' THEN 'YES' END), MAX(LEASES."REFUNDABLE")) AS "REFUNDABLE",
